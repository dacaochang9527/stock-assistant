...
- `selection/generate_d3_candidates.py`
  ...
  - 输出 `reports/daily/{D3_LABEL}_candidate_scan_{HHMMSS}.md` 和 `data/watchlists/{D3_LABEL}_watch_scan_{HHMMSS}.csv`。
  - 加 `--d1-only` 时，同时输出 `{D3_LABEL}_D1_filtered_{HHMMSS}.md/.csv`。
